...
# Description: This generates a dataframe to be written to a CSV containing one-sided 
# estimates, parameter values, standard errors, and other statistics of interest.
#-------------------------------------------------------------------------------------#
"format.output" <- function(country.estimation, one.sided.est.country, real.rate.country, start, end, run.se = TRUE) {
  ...
  if (run.se) {
    ...
    output.country[13, sec2] <- "Draws excluded:"
    output.country[13, (sec2 + 2)] <- country.estimation$out.stage3$se$number.excluded
    output.country[13, (sec2 + 3)] <- "Total:"
    output.country[13, (sec2 + 4)] <- niter
    output.country[14, sec2] <- "Percent excluded:"
    output.country[14, (sec2 + 2)] <- as.numeric(output.country[13, (sec2 + 2)]) / (as.numeric(output.country[13, (sec2 + 2)]) + as.numeric(output.country[13, (sec2 + 4)]))
    output.country[15, sec2] <- "Draws excluded because a_r > -0.0025:"
    output.country[15, (sec2 + 4)] <- country.estimation$out.stage3$se$number.excluded.a.r
    output.country[16, sec2] <- "Draws excluded because b_y <  0.025:"
    output.country[16, (sec2 + 4)] <- country.estimation$out.stage3$se$number.excluded.b.y
    output.country[17, sec2] <- "Draws excluded because a_y1 + a_y2 < 1:"
    output.country[17, (sec2 + 4)] <- country.estimation$out.stage3$se$number.excluded.a1a2
  }

  # Signal-to-noise ratios
  output.country[19, sec2] <- "Signal-to-noise Ratios"
  output.country[20, sec2] <- "lambda_g"
  output.country[20, (sec2 + 1)] <- country.estimation$lambda.g
  output.country[21, sec2] <- "lambda_z"
  output.country[21, (sec2 + 1)] <- country.estimation$lambda.z
  output.country[19, (sec2 + 4)] <- "Log Likelihood"
  output.country[20, (sec2 + 4)] <- country.estimation$out.stage3$log.likelihood
