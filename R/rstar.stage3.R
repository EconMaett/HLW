...
"rstar.stage3" <- function(log.output,
  ...
  #----------------------------------------------------------------------------#
  y.data <- cbind(
    100 * log.output[5:(t.end + 4)],
    inflation[5:(t.end + 4)]
  )
  x.data <- cbind(
    100 * log.output[4:(t.end + 3)],
  ...
  if (any(is.na(P.00.stage1))) {
    ...
    P.00 <- calculate.covariance(
      initial.parameters = initial.parameters, theta.lb = theta.lb, theta.ub = theta.ub,
  ...
  f <- function(theta) {
    return(-log.likelihood.wrapper(
      parameters = theta, y.data = y.data, x.data = x.data, stage = stage,
      lambda.g = lambda.g, lambda.z = lambda.z, xi.00 = xi.00, P.00 = P.00,
      use.kappa = use.kappa, kappa.inputs = kappa.inputs,
      param.num = param.num
    )$ll.cum)
  }
  nloptr.out <- nloptr(initial.parameters, f,
  ...
  log.likelihood <- log.likelihood.wrapper(
    parameters = theta, y.data = y.data, x.data = x.data, stage = stage,
    lambda.g = lambda.g, lambda.z = lambda.z, xi.00 = xi.00, P.00 = P.00,
    use.kappa = use.kappa, kappa.inputs = kappa.inputs, param.num = param.num
  )$ll.cum
  ...
  states <- kalman.states.wrapper(
    parameters = theta, y.data = y.data, x.data = x.data, stage = stage,
    lambda.g = lambda.g, lambda.z = lambda.z, xi.00 = xi.00, P.00 = P.00,
    use.kappa = use.kappa, kappa.inputs = kappa.inputs, param.num = param.num
  )

  matrices <- unpack.parameters.stage3(
    parameters = theta, y.data = y.data, x.data = x.data,
    lambda.g = lambda.g, lambda.z = lambda.z, xi.00 = xi.00, P.00 = P.00,
    use.kappa = use.kappa, kappa.inputs = kappa.inputs, param.num = param.num
  )
  ...
  if (run.se) {
    ptm <- proc.time()
    se <- kalman.standard.errors(
      t.end = t.end, states = states, theta = theta, y.data = y.data, x.data = x.data, stage = stage,
      ...
      use.kappa = use.kappa, kappa.inputs = kappa.inputs,
      param.num = param.num
    ) # optional additional parameter: params.exclude.add = c('param name')
