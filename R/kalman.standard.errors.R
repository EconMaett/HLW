...
"kalman.standard.errors" <- function(t.end, states, theta, y.data, x.data, stage,
  ...
  if (date.se < start.phi.se) {
    params.exclude.list <- c(params.exclude.list, "phi")
  } # If date is before 2020Q1, exclude phi from SE computation
  if (!is.na(fix.phi)) {
    params.exclude.list <- c(params.exclude.list, "phi")
  } # If fix.phi is not NA, exclude phi from SE computation
  if (use.kappa) {
    for (k in 1:(dim(kappa.inputs)[1])) {
      if (kappa.inputs$lower.bound[k] == kappa.inputs$upper.bound[k]) {
        params.exclude.list <- c(params.exclude.list, kappa.inputs$name[k])
      } # If kappa is fixed (upper bound = lower bound), exclude from SE computation
    }
  }
  if (!is.na(params.exclude.add)) {
    params.exclude.list <- c(params.exclude.list, params.exclude.add)
  } # option to input list of additional parameters to exclude
  ...
  log.likelihood.estimated.vector <- log.likelihood.wrapper(
    parameters = theta, y.data = y.data, x.data = x.data, stage = 3,
  ...
  for (l in 1:n.state.vars) {
    if (eigenvectors.pp1[1, l] < 0) {
      eigenvectors.pp1[, l] <- -eigenvectors.pp1[, l]
    }
  }
  ...
  for (i in params.se) {
    ...
    likelihood.gradient[, i] <- (log.likelihood.wrapper(
      parameters = d.theta, y.data = y.data, x.data = x.data, stage = 3,
      lambda.g = lambda.g, lambda.z = lambda.z, xi.00 = xi.00, P.00 = P.00,
      use.kappa = use.kappa, kappa.inputs = kappa.inputs,
      param.num = param.num
    )$ll.vec - log.likelihood.estimated.vector) / delta
  }
  ...
  for (l in 1:(n.params.se)) {
    if (eigenvectors.info[1, l] < 0) {
      eigenvectors.info[, l] <- -eigenvectors.info[, l]
    }
  }
  ...
  while (good.draws < niter) {
    ...
    if ((theta.i[param.num["a_r"]] <= a.r.constraint) &
      ...
      states.i <- kalman.states.wrapper(
        parameters = theta.i, y.data = y.data, x.data = x.data, stage = 3,
